...
def image_files_to_tfrecords(list_ds, output_folder, num_shard, single_photo):
    start = time.time()
    for shard_id in range(0, num_shard):
        shard_list = list_ds.shard(num_shards=num_shard, index=shard_id)
        if single_photo == False:
            read_image_file_write_tfrecord(shard_list, output_folder + "/part-" + "{:03d}-db".format(shard_id) + ".tfrecord")
            print("Shard " + str(shard_id) + " saved after " + str(int(time.time() - start)) + "s")
        else:
            read_image_file_write_tfrecord(shard_list, output_folder + "/part-search.tfrecord")
            print("Shard " + str(shard_id) + " saved after " + str(int(time.time() - start)) + "s")   
...
def tfrecords_to_write_embeddings(tfrecords_folder, output_folder, model, batch_size, single_photo=True):
    if single_photo == False:
        tfrecords = [
            f.numpy().decode("utf-8") for f in tf.data.Dataset.list_files(tfrecords_folder + "/*db.tfrecord", shuffle=False)
        ]
    else: 
        tfrecords = [
            f.numpy().decode("utf-8") for f in tf.data.Dataset.list_files(tfrecords_folder + "/*search.tfrecord", shuffle=False)
        ]
    start = time.time()
    for shard_id, tfrecord in enumerate(tfrecords):
        ...
        if single_photo == False:
            save_embeddings_ds_to_parquet(
                embeddings, shard, output_folder + "/part-" + "{:03d}-db".format(shard_id) + ".parquet"
            )
            print("Shard " + str(shard_id) + " saved after " + str(int(time.time() - start)) + "s")
        else:
            save_embeddings_ds_to_parquet(
                embeddings, shard, output_folder + "/part-search.parquet"
            )
            print("Shard " + str(shard_id) + " saved after " + str(int(time.time() - start)) + "s")
...
def compute_save_embeddings(list_ds, folder, num_shards, model, batch_size, single_photo):
    start = time.time()
    for shard_id in range(0, num_shards):
        ...
        print("Shard " + str(shard_id) + " done after " + str(int(time.time() - start)) + "s")
        if single_photo == False:
            save_embeddings_ds_to_parquet(embeddings, shard, folder + "/part-" + "{:03d}-db".format(shard_id) + ".parquet")
        else:
            save_embeddings_ds_to_parquet(embeddings, shard, folder + "/part-search.parquet")
        print("Shard " + str(shard_id) + " saved after " + str(int(time.time() - start)) + "s")
    print("Total time : " + str(int(time.time() - start)))


def run_inference_from_files(image_folder, output_folder, batch_size=1000, single_photo=True):
